    ...
    GaussianRasterizationSettings raster_settings)
{
    auto rasterization_result = RasterizeGaussiansCUDA(
        ...
        raster_settings.mip_filter_var_  // ← 新增参数
    );
    ...
    auto depth = std::get<4>(rasterization_result);   // ← 新增
    auto radii = std::get<5>(rasterization_result);   // ← 索引调整
    ...
    ctx->save_for_backward({raster_settings.bg_,
                            ...
                            imgBuffer,
                            sampleBuffer});
    return {color, radii, final_T,depth};
}
