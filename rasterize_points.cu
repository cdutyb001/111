...
) 
{
    ...
    if(P != 0) 
    {
        auto tup = CudaRasterizer::Rasterizer::forward(
            ...
            out_depth.contiguous().data<float>(),   // ← 新增：传递深度输出指针
    ...
    // 修改为（10个元素，新增 out_depth）
    return std::make_tuple(rendered, num_buckets, out_color, out_final_T, out_depth,
