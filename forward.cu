...
__device__ float4 computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, 
								...
								float mip_filter_var_ = 0.3f, 		// 新增参数，默认保持原行为 == version 1
								float gaussian_uncertainty = 0.0f,	// 该高斯的不确定性  == 新增参数，	version 2
								float uncertainty_beta = 3.0f,		// 不确定性权重      == 新增参数   	version 2	
								bool enable_probabilistic = false  	// 是否启用概率模式  == 新增参数   	version 2
							)
{
	...
	// step 1:计算原始2D协方差的行列式（滤波前）
	float det_orig = cov[0][0]*cov[1][1] - cov[0][1]*cov[0][1];
	det_orig = max(det_orig,1e-6f);  // 防止除零

	// ================Probabilistic Gaussian-LIC===================== version 2 ↓
	// step2 计算自适应滤波方差
	// 基础方差+不确定性调制
	float adaptive_filter_var = mip_filter_var_;

	if(enable_probabilistic){
		// Σ' = Σ + (s + β·U)·I
        // 不确定性越高，滤波越强（高斯越模糊）
		adaptive_filter_var +=uncertainty_beta * gaussian_uncertainty;
	}

	// ================Probabilistic Gaussian-LIC===================== version 2 ↑
	
	// step3 应用低通滤波器
	cov[0][0] += adaptive_filter_var;
	cov[1][1] += adaptive_filter_var;

	// step 4 计算滤波后的行列式
	float det_filtered = cov[0][0] * cov[1][1] - cov[0][1]*cov[0][1];
	det_filtered = max(det_filtered,1e-6f);
	
	// step 5 计算能量补偿系数 c = sqrt(det_orig / det_filtered)
	// 修改前
	// float compensation = sqrtf(det_orig/det_filtered);
	// compensation = min(max(compensation, 0.0f), 1.0f);

	// 修改后 - 使用更平滑的补偿函数
	float ratio = det_orig / det_filtered;
	float compensation;
	if(enable_probabilistic && gaussian_uncertainty > 0.3f){
		// 对高不确定性区域使用更保守的补偿
		compensation = powf(ratio, 0.4f);  // 减弱补偿强度
	} else {
		compensation = sqrtf(ratio);
	}
	compensation = min(max(compensation, 0.1f), 1.0f);  // 下限提高到0.1防止完全消失
	...
	return {float(cov[0][0]), float(cov[0][1]), float(cov[1][1]),compensation};
	//  ================Mip_Splat_part===================== version 1
}
...
__global__ void preprocessCUDA(int P, int D, int M,
	...
	float mip_filter_var,// 新增参数
	const float* uncertainties,//新增参数
	float uncertainty_beta,// 新增参数
	bool enable_probabilistic//新增参数
)  
	
{
	...
	// 获取当前高斯的不确定性
	float gaussian_uncertainty = 0.0f;
	if (enable_probabilistic && uncertainties != nullptr)
	{
		gaussian_uncertainty = uncertainties[idx];
	}
	
	// 计算带不确定性调制的2D协方差
	float4 cov_comp =  computeCov2D(
		p_orig, focal_x, focal_y, tan_fovx, tan_fovy, 
		limx_neg, limx_pos, limy_neg, limy_pos, 
		cov3D, viewmatrix, 
		mip_filter_var,
	...
	float3 cov = {cov_comp.x, cov_comp.y, cov_comp.z};
	float compensation = cov_comp.w; // 能量补偿系数
	...
	// const float4 co = { conic.x, conic.y, conic.z, opacities[idx] };
	 const float4 co = { conic.x, conic.y, conic.z, opacities[idx] * compensation };
...
renderCUDA(
	...
	const float* __restrict__ depths,           // 每个高斯的深度值
	...
	float* __restrict__ out_depth,              // ← 新增：深度输出
	bool no_color) 
{
	...
	__shared__ float collected_depth[BLOCK_SIZE];  // ← 新增：收集深度
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) 
	{
		...
		if (range.x + progress < range.y) 
		{
			...
			collected_depth[block.thread_rank()] = depths[coll_id];  // ← 新增
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) 
		{
			...
			// ← 新增：深度累积 (期望深度 = Σ alpha_i * T_i * depth_i)
	...
	if (inside) 
	{
		...
		// out_depth[pix_id] = D;  // ← 新增：写入深度
		out_final_T[pix_id] = T;
		float weight_sum = 1.0f - T;
		// out_depth[pix_id] = (weight_sum > 1e-6f) ? (D / weight_sum) : 0.0f;
		// forward.cu:560行修改为
		out_depth[pix_id] = (weight_sum > 1e-6f) ? 
							fminf(fmaxf(D / weight_sum, 0.1f),200.0f) : 0.0f;
		if (!no_color) 
		{
...
)
{
	preprocessCUDA<NUM_CHAFFELS> <<<(P + 255) / 256, 256>>> (
		...
		grid,
		tiles_touched,
		prefiltered, no_color,mip_filter_var,
